 ...
 * @param params
 */
export function log(_cfg: { silent: boolean; }, ...params: any[]) {
	if (!_cfg.silent) {
 ...
 * @param filename
 */
export function isEligible(_cfg: { exclude: any[]; }, filename: string) {
	return !_cfg.exclude.some((e: { test: (arg0: string) => any; }) => e.test(filename));
}
